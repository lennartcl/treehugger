...
exports.parse = function(s, useTypeAnnos) {
    var result = parser.parse(s, false, true, useTypeAnnos);
    ...
    switch(nodeName) {
        ...
        case "var":
            resultNode = tree.cons("VarDecls", [tree.list(n[1].map(function(varNode) {
                ...
                var result = varNode[2]
                    ? tree.cons("VarDeclInit", [idNode, transform(varNode[2])])
                    : tree.cons("VarDecl", [idNode]);
                result.typeAnno = varNode[1] ? transform(varNode[1]) : undefined;
                return result;
            }))]);
            break;
        case "let":
            resultNode = tree.cons("LetDecls", [tree.list(n[1].map(function(varNode) {
                ...
                setIdPos(varNode[0], idNode);
                var result = varNode[2]
                    ? tree.cons("LetDeclInit", [idNode, transform(varNode[2])])
                    : tree.cons("LetDecl", [idNode]);
                result.typeAnno = varNode[1] ? transform(varNode[1]) : undefined;
                return result;
            }))]);
            break;
        case "const":
            resultNode = tree.cons("ConstDecls", [tree.list(n[1].map(function(varNode) {
                ...
                setIdPos(varNode[0], idNode);
                var result = varNode[2]
                    ? tree.cons("ConstDeclInit", [idNode, transform(varNode[2])])
                    : tree.cons("ConstDecl", [idNode]);
                result.typeAnno = varNode[1] ? transform(varNode[1]) : undefined;
                return result;
            }))]);
            break;
        case "num":
            ...
            break;
        case "type":
            resultNode = tree.cons("Type", [tree.string(n[1][1] || n[1]), tree.string(n[2])]);
            break;
        case "string":
        ...
        case "object":
            resultNode = tree.cons("ObjectInit", [tree.list(n[1].map(function(propInit) {
                var value1 = transform(propInit[1]);
                var result;
                if (propInit[2]) {
                    result = tree.cons("PropertyInit", [tree.string(propInit[0]), transform(propInit[2])]);
                    result.typeAnno = value1;
                } else {
                    result = tree.cons("PropertyInit", [tree.string(propInit[0]), value1]);
                }
                return result;
            }))]);
            break;
        ...
        case "defun":
            resultNode = tree.cons("Function", [setIdPos(n[1], tree.string(n[1].name || "")), tree.list(n[2].map(function(arg) {
                var result = setIdPos(arg[1], tree.cons("FArg", [tree.string(arg[1].name)]));
                result.typeAnno = transform(arg[2]);
                return result;
            })), tree.list(n[4].map(transform))]);
            resultNode.typeAnno = transform(n[3]);
            break;
        case "function":
            ...
            resultNode = tree.cons("Function", [funName, fargs, tree.list(n[4].map(transform))]);
            resultNode.typeAnno = transform(n[3]);
            break;
        case "binary":
